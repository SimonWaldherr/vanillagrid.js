...
(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.VanillaGrid = factory());
    ...
    class VanillaGrid {
        ...
        constructor(container, options) {
            ...
            this._onDocClickTreeFilter = null;
            this._onToolbarTreeClick = null;
            this._onBodyClick = null;
            this._onBodyDblClick = null;
            this._onBodyContextMenu = null;
            this._onBodyKeyDown = null;
            this._onHeaderClick = null;
            this._onHeaderMouseDown = null;
            this._onToolbarClick = null;
            this._onToolbarChange = null;
            this._onPagerClick = null;
            this._resizeState = null;
            this._onDocClickSettings = null;
            this._initialContainerHtml = '';
            this._initialClassName = '';
            const el = (typeof container === 'string')
                ? document.querySelector(container)
                : container;
            if (!el)
                throw new Error('VanillaGrid: container not found');
            this.container = el;
            // Options
            const defaults = {
                data: [],
            ...
            if (options && options.tree)
                this.opts.tree = Object.assign({}, defaults.tree, options.tree);
            if (options) {
        ...
        setFilter(text, options = {}) {
            ...
            if (options.regexMode != null)
                this.state.regexMode = !!options.regexMode;
            if (options.caseSensitive != null)
                this.state.caseSensitive = !!options.caseSensitive;
            this.state.page = 1;
        ...
        setSort(key, dir = 'asc') {
            ...
            this._renderHeader();
            if (this._shouldRemoteFetch('sort'))
                this._remoteLoad(1);
            else
                this._renderBody();
        }
        setPageSize(size) {
            ...
            this.state.page = 1;
            this._renderPager();
            if (this._shouldRemoteFetch('page'))
                this._remoteLoad(1);
            else
                this._renderBody();
        }
      ...
      `;
        ...
        _renderToolbar() {
            ...
            const groupOptions = [`<option value="">${this._esc(i18n.noGroup)}</option>`]
                .concat(this.columns.map(c => `<option value="${this._esc(c.key)}" ${this.state.groupBy === c.key ? 'selected' : ''}>${this._esc(i18n.groupByPrefix)} ${this._esc(c.label ?? c.key)}</option>`));
      ...
      `;
            ...
            if (renderColumnsHere)
                this._bindColumnsMenuEvents(this.toolbarEl);
        ...
        _getThemeDefs() {
            ...
            for (const def of source) {
                if (!def.value || seen.has(def.value))
                    continue;
                defs.push({
                    value: def.value,
        ...
        _styleControlsMarkup() {
            ...
            const themeOptions = themes.map(def => `<option value="${this._escAttr(def.value)}"${def.value === currentTheme ? ' selected' : ''}>${this._esc(def.label)}</option>`).join('');
        ...
        _bindStyleControls(container) {
            if (!this._styleControlsEnabled() || !container)
                return;
        ...
        _syncStyleControlsUI() {
            if (!this._styleControlsEnabled() || !this.root || !this.styleState)
                return;
            ...
            wrappers.forEach(wrapper => {
                ...
                if (densitySelect)
                    densitySelect.value = this.styleState.density === 'compact' ? 'compact' : 'comfortable';
                const stripedToggle = wrapper.querySelector('.vg-settings-striped');
                if (stripedToggle)
                    stripedToggle.checked = !!this.styleState.striped;
            });
        }
        _applyFilter() {
            const input = this.toolbarEl.querySelector('.vg-filter-input');
        ...
        _renderTreeFilter() {
            ...
            const tree = Object.entries(columnStats)
                ...
                const valueItems = Object.entries(values)
                    ...
                    .map(([value, count]) => `<div class="vg-tree-filter-value" data-column="${colKey}" data-value="${this._escAttr(value)}">
        ...
        _columnsMenuConfig() {
            if (this.opts.columnsMenu === false)
                return null;
            const defaults = {
        ...
        _columnsMenuMarkup(cfg) {
            ...
            const cols = this.columns.filter(c => {
                if (includeSet && !includeSet.has(c.key))
                    return false;
                if (excludeSet && excludeSet.has(c.key))
                    return false;
                return true;
            });
        ...
        _bindColumnsMenuEvents(scopeEl) {
            ...
            toggles.forEach(chk => {
                chk.onchange = () => {
                    const key = chk.value;
                    if (chk.checked)
                        this.state.hiddenCols.delete(key);
                    else
                        this.state.hiddenCols.add(key);
                    this._applyColumnVisibility();
            ...
            if (selAll) {
                selAll.onchange = () => {
                    ...
                    (this.columns || []).forEach(c => {
                        if (check)
                            this.state.hiddenCols.delete(c.key);
                        else
                            this.state.hiddenCols.add(c.key);
                    });
                    this._applyColumnVisibility();
                    if (typeof this.opts.onColumnsVisibilityChange === 'function') {
                        this.opts.onColumnsVisibilityChange(new Set(this.state.hiddenCols));
        ...
        _renderHeader() {
            ...
            this.columns.forEach((col, idx) => {
                ...
                if (this.opts.sortable && (col.sortable !== false)) {
                    ...
                    if (sortKey === col.key)
                        th.classList.add(sortDir === 'asc' ? 'vg-asc' : 'vg-desc');
                }
            ...
            this.theadEl.querySelectorAll('.vg-th.vg-sortable').forEach(th => {
                th.onclick = (e) => {
                    ...
                    if (e.target.closest('.vg-resize-handle'))
                        return;
        ...
        _bindTableEvents() {
            ...
            this.tableEl.addEventListener('click', (e) => {
                ...
                if (grp) {
                    ...
                    if (this.state.collapsed.has(gval))
                        this.state.collapsed.delete(gval);
                    else
                        this.state.collapsed.add(gval);
        ...
        _bindMobileTouch() {
            ...
            if (tableWrap) {
                ...
                tableWrap.addEventListener('touchend', (e) => {
                    if (touchState.scrolling || touchState.swiping)
                        return;
                    ...
                    if (Math.abs(deltaX) > touchState.threshold && deltaY < 30) {
                        ...
                        if (deltaX < -50 && this.opts.pagination) {
                            this._nextPage();
                        }
        ...
        _startCellEdit(cell) {
            ...
            if (!column || column.type === 'button' || column.type === 'image')
                return;
        ...
        _mdInlineToHtml(src) {
            ...
            // images ![alt](url)
            s = s.replace(/!\[([^\]]*)\]\(([^\)\s]+)(?:\s+"([^"]+)")?\)/g, (m, alt, url) => `<img src="${this._escAttr(url)}" alt="${this._escAttr(alt)}" loading="lazy" class="vg-img" />`);
            // links [text](url)
            s = s.replace(/\[([^\]]+)\]\(([^\)\s]+)(?:\s+"([^"]+)")?\)/g, (m, text, url) => `<a href="${this._escAttr(url)}" target="_blank" rel="noopener noreferrer" class="vg-link">${text}</a>`);
            // bold/italic/code
            s = s.replace(/\*\*([^*]+)\*\*/g, '<b>$1</b>')
                .replace(/\*([^*]+)\*/g, '<i>$1</i>')
        ...
        _renderRow(row, depth = 0, meta = null) {
            ...
            if (this.opts.selectable) {
                ...
                cb.onchange = () => {
                    if (cb.checked)
                        this.state.selected.add(row.__vgid);
                    else
                        this.state.selected.delete(row.__vgid);
                    this._updateHeaderSelectionToggle();
                    this._emitSelection();
                };
        ...
        _renderCell(td, row, col) {
            ...
            switch (type) {
                ...
                case 'image': {
                    ...
                    if (col.image && col.image.width)
                        img.width = col.image.width;
                    if (col.image && col.image.height)
                        img.height = col.image.height;
                    img.loading = 'lazy';
        ...
        _renderAggSummary(group) {
            ...
            numericCols.forEach(col => {
                ...
                const bits = [];
                if (want.includes('sum') && ag.sum != null)
                    bits.push(`Σ ${this._fmtAgg(col, ag.sum)}`);
                if (want.includes('min') && ag.min != null)
                    bits.push(`min ${this._fmtAgg(col, ag.min)}`);
                if (want.includes('max') && ag.max != null)
                    bits.push(`max ${this._fmtAgg(col, ag.max)}`);
                if (bits.length) {
        ...
        _renderPager() {
            ...
            const sizeOptions = this.opts.pageSizes.map(n => `<option value="${n}" ${+this.state.pageSize === +n ? 'selected' : ''}>${n}${this._esc(this.opts.i18n.perPageSuffix)}</option>`).join('');
            const colMenuCfg = this._columnsMenuConfig();
      ...
      `;
            ...
            this.pagerEl.querySelector('[data-action="first"]').onclick = () => {
                if (this.state.page !== 1) {
                    this.state.page = 1;
                    if (remote)
                        this._remoteLoad(this.state.page);
                    else {
                        this._renderBody();
                        this._renderPager();
                    }
        ...
        _process() {
            if (this.opts.tree.enabled) {
                const flat = this._filterSortTreeFlatten();
                if (!this.opts.pagination)
                    return { hasGroups: false, pageFlatRows: flat };
        ...
        _filterSortTreeFlatten() {
            ...
            if (f) {
                if (this.state.regexMode) {
                    try {
                        matcher = { type: 'regex', rx: new RegExp(f, 'i') };
                    ...
                    this.state.regexError = '';
                    matcher = { type: 'text', needle: f.toLowerCase() };
                }
            }
            ...
            const matchesRow = (row) => {
                ...
                if (matcher.type === 'regex')
                    return this.columns.some(c => matcher.rx.test(this._cellText(row, c)));
            ...
            const mark = (node, parents) => {
                ...
                kids.forEach(ch => { if (mark(ch, parents.concat(node)))
                    ok = true; });
                if (!matcher || ok) {
                    visible.add(node);
            ...
            const trav = (nodes, depth) => {
                ...
                nodes.forEach(n => {
                    ...
                    if (expanded && Array.isArray(n[ck]))
                        trav(n[ck], depth + 1);
                });
            };
            trav(roots, 0);
        ...
        _assignIdsDeep(list) {
            ...
            list.forEach(row => {
                if (row.__vgid == null)
                    row.__vgid = this._idCounter++;
                if (Array.isArray(row[ck]))
                    this._assignIdsDeep(row[ck]);
            });
        }
        _indexRowsDeep(list) {
            if (!Array.isArray(list))
        ...
        _shouldRemoteFetch(why) {
            if (typeof this.opts.loadPage !== 'function')
                return false;
            if (this.opts.serverPagination)
                return true;
            if (this.opts.serverSorting && why === 'sort')
                return true;
            if (this.opts.serverFiltering && why === 'filter')
                return true;
            if (why === 'init')
                return this.opts.serverPagination || this.opts.serverSorting || this.opts.serverFiltering;
            if (why === 'page')
                return this.opts.serverPagination;
            return false;
        }
        _processMetaRemote() {
        ...
        _computeAggregates(rows) {
            const out = {};
            this.columns.forEach(col => {
                if (!Array.isArray(col.aggregations) || col.aggregations.length === 0)
                    return;
                const nums = rows
                ...
                if (!nums.length)
                    return;
                const agg = {};
                if (col.aggregations.includes('sum'))
                    agg.sum = nums.reduce((a, b) => a + b, 0);
                if (col.aggregations.includes('min'))
                    agg.min = Math.min(...nums);
                if (col.aggregations.includes('max'))
        ...
        _updateFilterError() {
            if (!this.opts.filterable || !this.toolbarEl)
                return;
        ...
        _currentPageRowIds() {
            ...
            if (this.opts.tree.enabled) {
                ...
                if (!hasGroups)
                    (pageRows || []).forEach(r => ids.push(r.__vgid));
                else
                    (pageGroups || []).forEach(g => g.rows.forEach(r => ids.push(r.__vgid)));
            }
            return ids;
        }
        ...
        _emitSelection() {
            if (typeof this.opts.onSelectionChange === 'function') {
                ...
                try {
                    this.opts.onSelectionChange(rows);
                }
                catch { }
            }
        }
        ...
        _renderPivotToolbar() {
            ...
            if (this.state.pivotControlsCollapsed)
                pivotControls.classList.add('collapsed');
      ...
      `;
        ...
        _getFieldType(field) {
            ...
            if (typeof sample === 'number')
                return 'number';
            if (sample instanceof Date || /^\d{4}-\d{2}-\d{2}/.test(sample))
                return 'date';
            return 'text';
        }
        ...
        _bindPivotEvents() {
            ...
            toolbar.querySelector('.vg-pivot-toggle')?.addEventListener('click', (e) => {
                ...
                if (wrapper)
                    wrapper.classList.toggle('collapsed', this.state.pivotControlsCollapsed);
                if (btn) {
...
//# sourceMappingURL=vanillagrid.umd.js.map
