...
The plain JavaScript source files are the canonical source. The build produces minified browser assets in `dist/`:
...
For development, load `vanillagrid.js` and `vanillagrid.css` directly. The library also supports CommonJS consumers:

```js
const VanillaGrid = require('./vanillagrid.js');
```
...
- Run `npm run check` to verify JavaScript syntax.
- Run `npm run build` to regenerate minified assets.
- Run `npm start` and open `http://127.0.0.1:8000/` to work through the demos.
## Run Locally
